...
class LibraryBookLease(models.TransientModel):
    ...
    borrower = fields.Many2one('library.book.borrower', string='Borrower', required=True)
    date_return = fields.Date(string='Return Date', required=True)
    books = fields.One2many('library.book', 'isbn', string='Books', readonly=True)

    def confirm(self):
        self.ensure_one()
        target_book = self.env['library.book'].search([('isbn', '=', self.book_isbn)])
        target_book.write({'current_borrower': self.borrower.id, 'status' : 'borrowed'})
